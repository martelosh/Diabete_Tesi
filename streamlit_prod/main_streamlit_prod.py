...
box-shadow:0 14px 30px rgba(0,0,0,.15);cursor:pointer}
...
def render_chat():
    ...
    if st.session_state.chat_open and st.session_state.last_pred is not None:
        ...
        if prompt := st.chat_input("Scrivi un messaggio…"):
            ...
            if not reply:
                sp = build_dynamic_system_prompt(st.session_state.last_pred or 0, st.session_state.last_prob or 0.0)
                llm = deepseek_chat(st.session_state.messages, system_prompt=sp)
                reply = llm if not (llm.startswith("⚠️") or llm.lower().startswith("errore")) \
                        else "Il consulente non è disponibile. Scrivi il tuo **comune** per i contatti utili."
